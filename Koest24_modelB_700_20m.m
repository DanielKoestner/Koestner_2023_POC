...
% dataset of only surface data described in Koestner et al. 2024.
...
function [POC,PI,comp]=Koest24_modelB_700_20m(bbp,chla,alignment,bias,alpha)
...
% % % % New model B coefficients from Koestner et al. 2024
k_modelB=[181.7663757089398; 0.381451549673783; 0.735667005938175; 0.140944959450922];
...
if bias(1:3)=='pow'
    bias_modelB_coef=[1.513053336292306, -0.793389486155043];
    bias_lim=35.2;
    pocB(pocB<bias_lim)=(pocB(pocB<bias_lim).^bias_modelB_coef(1))*10^bias_modelB_coef(2);
elseif bias(1:3)=='lin'
    bias_modelB_coef=[1.508426682346915, -16.948934799216836];
    bias_lim=33.3;
...
S=[0.041825428392709	0.016465995743229	-0.018690006928174	-0.007312694626357
0.016465995743229	0.006866013055458	-0.007377549048522	-0.003048923534070
-0.018690006928174	-0.007377549048522	0.008898891487657	0.003458386490342
-0.007312694626357	-0.003048923534070	0.003458386490342	0.001416314807959]; %covariance matrix of coefficients

mse=0.021259582191403; % mean square error from regression model, corrected 20260108
